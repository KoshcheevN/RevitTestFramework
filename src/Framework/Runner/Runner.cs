...
namespace RTF.Framework
{
    ...
    public partial class Runner : IRunner, IDisposable
    {
        ...
        /// </summary>
        private List<string> CopiedAddins { get; set; } = new List<string>();
        ...
        /// </summary>
        [XmlIgnore]
        public ObservableCollection<IAssemblyData> Assemblies { get; set; } = new ObservableCollection<IAssemblyData>();

        /// <summary>
        /// A collection of available Revit products for testing.
        /// </summary>
        [XmlIgnore]
        public ObservableCollection<RevitProduct> Products { get; set; } = new ObservableCollection<RevitProduct>();
        ...
        /// </summary>
        public bool CopyAddins { get; set; } = true;
        ...
        /// </summary>
        public List<string> AdditionalResolutionDirectories { get; set; } = new List<string>();
        ...
        private bool CreateJournalForTestCases(IEnumerable<ITestData> runnableTests)
        {
            ...
            if (GroupByModel)
            {
                var groups = runnableTests.GroupBy(x => x.ModelPath).ToList();

                foreach (var group in groups)
                {
                    foreach (var test in group)
                    {
                        ModelSemantics modelSemantics = ModelSemantics.DoNothing;

                        if (test == group.First())
                        {
                            modelSemantics |= ModelSemantics.Open;
                        }
                        if (test == group.Last())
                        {
        ...
        private void AddToJournal(string path, string testName, string fixtureName, string assemblyPath, string resultsPath, string modelPath, ModelSemantics modelSemantics)
        {
            using (var tw = new StreamWriter(path, true))
            {
                string journal = "";

                if (modelSemantics.HasFlag(ModelSemantics.Open))
                {
                    journal += $"Jrn.Command \"StartupPage\" , \"Open this project , ID_FILE_MRU_FIRST\" \n";
                ...
                {
                    journal += $"Jrn.Command \"Internal\" , \"Close the active project , ID_REVIT_FILE_CLOSE\" \n";
                }
            ...
            ModelSemantics modelSemantics = ModelSemantics.Open | ModelSemantics.Close)
        {
            ...
            try
            {
                ...
                if (continuous)
                {
                    ...
                    AddToJournal(journalPath, td.Name, td.Fixture.Name, td.Fixture.Assembly.Path, Results, td.ModelPath, modelSemantics);
                }
                else
                {
        ...
        public virtual IList<IAssemblyData> ReadAssembly(string assemblyPath, string workingDirectory, GroupingType groupType, bool isTesting)
        {
            ...
            try
            {
                ...
                if (!isTesting)
                {
                    ...
                    var revitProductReferenced = (assData.ReferencedRevitVersion != null) ? Products.FirstOrDefault(x => x.Name.Contains(assData.ReferencedRevitVersion)) : null;
